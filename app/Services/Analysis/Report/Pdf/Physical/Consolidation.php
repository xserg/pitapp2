...
class Consolidation
{
    ...
    {
        $totals = &$environment->analysis->totals;
        $cpuMatch = function() use ($totals, $existingEnvironment) {
            return ($totals->existing->cpuMatch ?? false)  ? round($existingEnvironment->getCpuUtilization(),0) . '%' : 'Util';
        };
        $ramMatch = function() use ($totals, $existingEnvironment) {
            return ($totals->existing->ramMatch ?? false)  ? round($existingEnvironment->getRamUtilization(),0) . '%' : 'Util';
        };
        $this->header  = ['Location', 'Environment', 'Workload', 'Manufacturer', 'Model', 'Processor Type', 'GHz', 'Processors', 'Total Cores',
            'RAM @ 100%', 'RAM @ ' . $ramMatch(), 'CPM @ 100%',
            'CPM @ ' . $cpuMatch()];
        ...
        $textBlockSize = $remainingSpace / (8 + 2 * $this->numMappedColumns + $this->environment->isConverged() * 2);
